...
BASE_REASONING_SYSTEM = (
    "You are Tscript AI, a precise instrument for text. You reason carefully before answering: "
    "silently work through what is actually being asked, what evidence you have (including any web search "
    "results or attached context supplied to you), and what could be wrong with a first-guess answer, then "
    "give the person only your final, well-structured answer. "
    "Be accurate over confident: if you are not sure of a fact, say so rather than inventing one. "
    "When web search results are supplied, ground claims in them and mention the source naturally (e.g. "
    "'according to [source]') rather than fabricating citations; do not claim to have searched the web if no "
    "results were supplied. Prefer clear structure (short paragraphs, headers, or lists) for anything with "
    "more than one part, but do not pad a simple answer with unnecessary sections. Match the person's tone and "
    "the complexity of the question - a quick factual question deserves a quick, direct answer; a genuinely "
    "hard or open-ended question deserves careful, layered reasoning. Never use filler phrases just to sound "
    "more advanced."
)
...
MODELS = {
    "standard": {
        "temperature": 0.4,
        "system": BASE_REASONING_SYSTEM + " Default mode: be practical, precise, and conversational.",
    },
    "fast": {
        "temperature": 0.2,
        "system": "You are Tscript AI in fast mode. Answer directly and correctly with minimal fluff. No preamble.",
    },
    "think_deep": {
        "temperature": 0.25,
        "system": BASE_REASONING_SYSTEM
        + " Deep analysis mode: this question deserves careful, multi-step reasoning. Break the problem into its "
        "parts, weigh evidence and trade-offs explicitly, address likely counterarguments or edge cases, and "
        "arrive at a well-justified conclusion. Cite specific evidence from any supplied context or search "
        "results rather than speaking in generalities.",
    },
    "advance": {
        "temperature": 0.3,
        "system": BASE_REASONING_SYSTEM
        + " Advanced mode: produce a detailed, polished, production-quality answer with clear sections, "
        "concrete specifics (numbers, names, steps) instead of vague generalities, and a short summary or "
        "recommendation where relevant.",
    },
    "annotation_expert": {
        "temperature": 0.1,
        "system": "You are Tscript AI. Focus on precise, standards-based annotation and labeling guidance.",
    },
}
...
SEARCH_TRIGGER_PATTERN = re.compile(
    r"\b(today|current|currently|latest|recent|recently|right now|this week|this month|this year|"
    r"news|breaking|update[sd]?|score|stock|price|weather|forecast|release date|who is the (?:current|new)|"
    r"20(2[4-9]|3[0-9]))\b",
    re.IGNORECASE,
)

IMAGE_GENERATE_PATTERN = re.compile(
    r"\b(generate|create|draw|make|design|render)\b[^.\n]{0,60}\b(image|picture|photo|illustration|artwork|logo|icon|wallpaper)\b",
    re.IGNORECASE,
)
IMAGE_SEARCH_PATTERN = re.compile(
    r"\b(find|show|search for|look up|get)\b[^.\n]{0,60}\b(image|picture|photo)s?\b",
    re.IGNORECASE,
)


def should_auto_search(message: str) -> bool:
    return bool(SEARCH_TRIGGER_PATTERN.search(message or ""))


def web_search(query: str, max_results: int = 5) -> List[Dict[str, str]]:
    """Keyless web search via DuckDuckGo's HTML endpoint. Best-effort; returns [] on any failure."""
    if not query.strip():
        return []
    try:
        resp = requests.post(
            ...
            data={"q": query},
            headers={"User-Agent": "Mozilla/5.0 (compatible; TscriptAI/1.0)"},
            timeout=15,
        )
        if resp.status_code >= 400:
            return []
        ...
        for node in soup.select(".result")[: max_results * 2]:
            ...
            if not link or not link.get("href"):
                continue
            title = trim_text(link.get_text(" ", strip=True), 160)
            snippet = trim_text(snippet_el.get_text(" ", strip=True), 320) if snippet_el else ""
            url = link.get("href")
...
def web_image_search(query: str, max_results: int = 6) -> List[Dict[str, str]]:
    """Best-effort keyless image search via DuckDuckGo. Returns [] on failure; caller should degrade gracefully."""
    if not query.strip():
        return []
    try:
        ...
        resp = requests.get(
            ...
            params={"q": query, "vqd": vqd_match.group(1), "f": ",,,", "l": "us-en"},
            headers={"User-Agent": "Mozilla/5.0 (compatible; TscriptAI/1.0)", "Referer": "https://duckduckgo.com/"},
            timeout=15,
        )
        data = parse_json_response(resp)
        results = []
        for item in (data.get("results") or [])[:max_results]:
            ...
            if not image_url:
                continue
            results.append({"title": trim_text(item.get("title") or "", 120), "url": image_url, "thumbnail": item.get("thumbnail") or image_url, "source": item.get("url") or ""})
        return results
    except Exception as exc:
        logger.warning("web_image_search failed: %s", exc)
        return []


def pollinations_image_url(prompt: str, seed: Optional[int] = None) -> str:
    import urllib.parse

    encoded = urllib.parse.quote(prompt.strip()[:800])
    seed_val = seed if seed is not None else secrets.randbelow(1_000_000)
    return f"https://image.pollinations.ai/prompt/{encoded}?width=1024&height=1024&seed={seed_val}&nologo=true"


YOUTUBE_URL_PATTERN = re.compile(r"(youtube\.com/(watch\?v=|shorts/|embed/)|youtu\.be/)", re.IGNORECASE)
...
def download_youtube_audio(url: str, dest_dir: Path) -> Tuple[Path, Dict[str, Any]]:
    ...
    if not audio_path.exists():
        ...
        if not candidates:
            raise http_error(500, "Audio extraction failed")
...
@app.post(route("/youtube/transcribe"))
def youtube_transcribe(request: Request, response: Response, payload: Dict[str, Any] = Body(...)) -> JSONResponse:
    enforce_rate_limit(request, "youtube")
    actor = actor_from_request(request, response)
    url = trim_text(str(payload.get("url") or ""), 500)
...
TEXT_IMPROVE_INSTRUCTIONS = {
    "grammar": "Correct grammar, spelling, and punctuation only. Preserve the author's wording, tone, and meaning as closely as possible. Do not summarize or add content.",
    "punctuation": "Improve punctuation and sentence boundaries only (commas, periods, capitalization, paragraph breaks). Do not change word choice.",
    "readability": "Lightly restructure sentences for clarity and readability while preserving meaning and tone. Fix run-ons and awkward phrasing.",
    "formal": "Rewrite in a clear, polished, professional tone suitable for a formal document, preserving all factual content.",
    "shorten": "Tighten and condense the text, removing redundancy, while preserving all key information.",
    "expand": "Expand the text with natural elaboration and smoother transitions, without inventing new facts.",
}


@app.post(route("/text/improve"))
def text_improve(payload: Dict[str, Any] = Body(...)) -> JSONResponse:
    text = str(payload.get("text") or "").strip()
    mode = str(payload.get("mode") or "grammar").strip().lower()
    if not text:
        raise http_error(400, "No text supplied")
    instruction = TEXT_IMPROVE_INSTRUCTIONS.get(mode, TEXT_IMPROVE_INSTRUCTIONS["grammar"])
    prompt = (
        f"{instruction}\n\nReturn only the corrected text with no preamble, no explanation, and no quotation marks "
        f"around it.\n\nText:\n{text[:24000]}"
    )
    improved = groq_chat([{"role": "user", "content": prompt}], mode="standard", max_tokens=3000, temperature=0.15)
    return json_ok({"text": improved.strip() or text, "mode": mode})
...
def chat(
    ...
    # --- Image intents: handled before the general chat call so we don't waste a chat round-trip. ---
    generated_image = None
    image_results: List[Dict[str, str]] = []
    if IMAGE_GENERATE_PATTERN.search(user_message):
        image_prompt = trim_text(user_message, 400)
        generated_image = {"url": pollinations_image_url(image_prompt), "prompt": image_prompt}
    elif IMAGE_SEARCH_PATTERN.search(user_message):
        image_results = web_image_search(user_message, max_results=6)

    # --- Web search grounding: explicit toggle from the client, or automatic for time-sensitive questions. ---
    search_results: List[Dict[str, str]] = []
    do_search = (web_search_flag == "on") or (web_search_flag == "auto" and settings.web_search_enabled and should_auto_search(user_message) and not generated_image)
    if do_search:
        search_results = web_search(user_message, max_results=5)

    search_context = ""
    if search_results:
        lines = [f"[{i+1}] {r['title']} — {r['snippet']} ({r['url']})" for i, r in enumerate(search_results)]
        search_context = "\n\nLive web search results (use these to ground your answer; today's date context applies):\n" + "\n".join(lines)

    history = recent_chat_history(actor)
    image_note = ""
    if generated_image:
        image_note = f"\n\n(An image has already been generated and shown to the user for this request: \"{generated_image['prompt']}\". Briefly acknowledge it; do not describe generating it as a future action.)"
    elif image_results:
        image_note = "\n\n(Relevant images have already been fetched and shown to the user above your reply. Briefly reference them; do not claim you cannot show images.)"

    messages = [*history, {"role": "user", "content": f"{user_message}{attachment_context}{search_context}{image_note}"}]
